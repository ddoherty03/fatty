...
module FatTerm
  module Logger
    ...
    #
    def self.configure(level: ::Logger::INFO, json: true, progname: "fat_term")
      path =
        if FatTerm::Config.config[:log_file]
          File.expand_path(FatTerm::Config.config[:log_file])
