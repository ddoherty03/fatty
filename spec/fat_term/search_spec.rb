...
RSpec.describe FatTerm::Search do
  describe ".split_terms" do
    it "splits on whitespace and drops empties" do
      expect(FatTerm::Search.split_terms("  foo   bar baz  ")).to eq(["foo", "bar", "baz"])
    end
  end

  describe ".match_all_terms?" do
    it "matches empty query" do
      expect(FatTerm::Search.match_all_terms?("foo bar", "")).to be(true)
    end

    it "matches one term" do
      expect(FatTerm::Search.match_all_terms?("foo bar", "foo")).to be(true)
    end

    it "matches multiple terms in any order" do
      expect(FatTerm::Search.match_all_terms?("foo bar baz", "baz foo")).to be(true)
    end

    it "fails when one term is missing" do
      expect(FatTerm::Search.match_all_terms?("foo bar baz", "foo qux")).to be(false)
    end

    it "matches case-insensitively" do
      expect(FatTerm::Search.match_all_terms?("Foo Bar", "foo bar")).to be(true)
    end
  end

  describe ".compile_regexp" do
    it "builds a regex that matches all terms in any order" do
      re = FatTerm::Search.compile_regexp("foo bar", regex: false)
    ...
    it "is case-insensitive when pattern has no uppercase letters" do
      re = FatTerm::Search.compile_regexp("foo", regex: false)
    ...
    it "passes regex patterns through unchanged when regex is true" do
      re = FatTerm::Search.compile_regexp("f.o", regex: true)
      expect("foo").to match(re)
    end
  end
end
