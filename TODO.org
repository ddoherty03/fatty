* TODO Bottom-aligned output
Render short output at the bottom of the output pane and let it grow upward,
so Fatty feels more like a conventional terminal.

This would require coordinated changes to both Renderer::Truecolor and
Renderer::Curses, especially the curses incremental-scroll path.

Reconsider at the same time whether Renderer::Curses is still worth
maintaining.  Truecolor works over SSH when forced on, but automatic detection
can be conservative in environments such as TERM=tmux-256color without
COLORTERM.

* TODO Horizontal Scrolling
* TODO Add modes for vi-like keybindings
- will require changing keybindings.yml format to include a mode qualifier
- for advanced two-step actions, e.g. dw, will require an action-pending
  capability
* TODO Add Dynamic help
One of these days I would like to add a HelpApi so that env.help prints a
dynamically-generated help text to output in markdown. It would know about the
user's keybindings, keydefs, and actions. It would get key descriptions from
the action desc line.

* TODO Allow count in keybindings
#+begin_src ruby
map.bind_mouse(
  context: :paging,
  button: :scroll_down,
...
With count: nil meaning “do not bake a count into the binding; let the normal
keyboard prefix count machinery apply.”

* DONE Name Space history and other files
CLOSED: [2026-07-14 Tue 05:30]
Allow Fatty::Terminal to take an app_name: argument so that the history path,
perhaps config, keybindings.

So this
#+begin_src ruby
Fatty::Terminal.new(app_name: "byr")
#+end_src
would result in a history path of =~/.local/state/fatty/apps/byr/history=.

Use an =apps= directory under the =fatty= directory to prevent name collisions
with general =fatty= files.

But this
#+begin_src ruby
Fatty::Terminal.new(
  app_name: "byr",
  history_path: "~/some/special/history.jsonl",
)
#+end_src
still allows the caller to override the app_name-based default.

This could also apply to log files, config files, and perhaps other things.

* DONE Fix numbers in search
CLOSED: [2026-06-18 Thu 04:04]
They are getting consumed as counts, I think.  They should be available for narrowing.
* DONE Theming
