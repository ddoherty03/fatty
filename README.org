...
* Usage
** Launching a Fatty Terminal with ~on_accept~m
...
as show in this file that we'll call ~loudrev~:
...
On running a =fatty= application, =fatty= installs a few files if they do not exists:
...
*** =environment=
This returns a =Hash= of various runtime conditions detected by =Fatty=:
...
field and assigning it one of the following "roles":
...
Display the text in the status area with the role :error. An alias for
=error(text)=

*** =alert(text, role: :info)=
Display the text in the one-line alert panel just below the input field.  In
the case of the alert panel, the role only controls the foreground color not
the background.
...
=choose= returns
- the user's selection as a String if =choices:= was an =Array= of =Strings=,
- the value from the =choices:= =Hash= if =choices:= was an =Hash=,
- or the =cancel_value= if the user canceled the selection with C-c of C-g.
...
Fatty::Terminal.new(
  on_accept: asker,
  prompt: "Type 'go' for some action> ",
).go
#+end_src
