...
The default user configuration directory follows =XDG_CONFIG_HOME= if it is
set.  Otherwise it is =~/.config/fatty=.
...
| keypad_8     | keypad_9     | keypad_divide | keypad_multiply | keypad_minus | keypad_plus | keypad_decimal |          |
...
| end          | page_up      | page_down     | up              | down         | left        |
| keypad_0     | keypad_1     | keypad_2      | keypad_3        | keypad_4     | keypad_5    |
| keypad_8     | keypad_9     | keypad_divide | keypad_multiply | keypad_minus | keypad_plus |
| keypad_enter | keypad_comma |               |                 |              |             |

Some terminals can report application-keypad sequences such as =keypad_divide=,
=keypad_multiply=, or =keypad_enter=.  Numeric keypad digits are often not
distinguishable from ordinary digits when NumLock is on, and are often reported
as navigation keys such as =end=, =down=, =page_down=, or =left= when NumLock is
off.  Use =keytest= to see what your terminal actually sends.
...
*** Contexts
The default context is =input=.  Other common contexts include =paging= and
=popup=.
...
A Fatty theme is a YAML file in =~/.config/fatty/themes=.  The file name does
not matter to the renderer, but the theme must have a =name= field, and the
name is what you put in =config.yml=:

#+begin_src yaml :tangle no
theme: nordic
#+end_src

A minimal theme can define only =output= and inherit everything else through
Fatty's role defaults:
...
A more useful theme defines the main editing, status, search, popup, and
markdown roles:
...
If a role does not specify =inherit=, Fatty uses its built-in role hierarchy.
For example, =input= inherits from =output=, =input_suggestion= inherits from
=input=, =popup_selection= inherits from =region=, and =warn= / =error= build
on the semantic status roles.
...
Themes can be changed while Fatty is running.  The default Emacs-style bindings
include:
