...
The =esc_meta= setting controls whether Fatty treats =ESC= followed by another
key as a Meta-key chord.  By default this is false, so a bare =ESC= remains a
responsive cancel key.
...
If you set =esc_meta: true=, =esc_delay= controls how long curses waits after
receiving =ESC= before deciding whether another key is part of the same
Meta-key sequence.  The value is in milliseconds.
...
recognized by name and modifiers (except for the strange behavior of
=terminator= on C-PageDown!).
